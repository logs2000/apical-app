...
import { PLAN_LIST } from '@/lib/platform/pricing'
...
export interface LandingPageProps {
  /** Called when the user clicks "Open the web app". */
  onLaunch?: () => void
}
...
// Re-export so the app shell can import it from the landing barrel.
export { clearLandingSeen } from './os-detect'
            ...
            <Sheet open={open} onOpenChange={setOpen}>
              ...
              <SheetContent side="right" className="w-72">
                <div className="mt-6 flex flex-col gap-4 px-4">
                  ...
                  <a href="/docs" onClick={() => setOpen(false)} className="text-sm font-medium">Docs</a>
...
function Hero({
  ...
  return (
    <section className="relative overflow-hidden">
      {/* Subtle background — no loud green blobs */}
      <div
        aria-hidden
      ...
      />
      <div className="dark:hidden" aria-hidden style={{ position: 'absolute', inset: 0, zIndex: -10 }}>
        {/* light-mode soft glow */}
      </div>

      <div className="mx-auto max-w-4xl px-4 py-20 text-center md:py-28 md:px-6">
        <motion.div
        ...
        >
          <div className="overflow-hidden rounded-xl border border-border/70 shadow-2xl shadow-black/5">
            {/* next/image not used for a static screenshot — plain img is fine */}
            <img
              src="/screenshot-dashboard.png"
              alt="Apical dashboard showing agents and their work"
              className="w-full"
              loading="eager"
            />
          </div>
...
// ---------------------------------------------------------------------------
// How it works — one section, three simple steps
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Use cases — brief, relatable, no jargon
// ---------------------------------------------------------------------------

function UseCases({ reduced }: { reduced: boolean | null }) {
  const cases = [
    { emoji: '📄', title: 'Filing', body: 'Sort scans, invoices, and documents into the right folders automatically.' },
    { emoji: '💼', title: 'Sales leads', body: 'Find potential customers and keep your list fresh without lifting a finger.' },
    { emoji: '🔔', title: 'Reminders', body: 'Never let a license, contract, or deadline expire again.' },
    { emoji: '📊', title: 'Competitor watch', body: 'Know when a competitor changes their pricing or launches something new.' },
    { emoji: '✉️', title: 'Inbox triage', body: 'Every inbound email sorted, routed, and answered the right way.' },
    { emoji: '🧾', title: 'Expenses', body: 'Audit reports against your policy and flag the ones that need a look.' },
  ]
                  ...
                  <Button
                    ...
                    onClick={() => {
                      if (isFree) onLaunch()
                        ...
                        // Start checkout (demo mode upgrades immediately).
...
// Cheap way to detect the current plan client-side (reads the cached sub from
// /api/billing/subscription if available). For the landing page we don't need
// this to be live — just a best-effort label.
const currentPlanId = 'free' // visitors on the landing are typically free
...
// ---------------------------------------------------------------------------
// For Developers — short, links to /docs
// ---------------------------------------------------------------------------
          ...
          <div className="grid grid-cols-2 gap-8 text-xs sm:grid-cols-4">
            ...
            <div>
              ...
              <ul className="space-y-2 text-muted-foreground">
                ...
                <li><a href="/docs" className="hover:text-foreground">MCP + API</a></li>
              </ul>
            </div>
            <div>
...
function ApicalMark({ className }: { className?: string }) {
  return (
    <svg viewBox="0 0 32 32" fill="none" className={className} aria-hidden="true">
      <defs>
        <linearGradient id="apicalGradMark" x1="0" y1="32" x2="32" y2="0" gradientUnits="userSpaceOnUse">
          <stop stopColor="oklch(0.55 0.09 155)" />
          <stop offset="1" stopColor="oklch(0.70 0.10 165)" />
        </linearGradient>
      </defs>
      <path d="M16 2 L29 28 L22.5 28 L16 14 L9.5 28 L3 28 Z" fill="url(#apicalGradMark)" />
      <circle cx="16" cy="6" r="2.2" fill="oklch(0.98 0.01 150)" />
    </svg>
  )
}

// ---------------------------------------------------------------------------
// Shared: Download button + dialog
// ---------------------------------------------------------------------------
...
function DownloadButton({
  ...
  // The button tries the real download first. If the binary isn't uploaded
  // (404 not_uploaded), it opens the dialog with the CLI fallback.
  const handleClick = async () => {
    if (os === 'other') {
      setOpen(true)
      return
    }
    // Hit the API. If it returns a file (200), the browser downloads it. If
    // it returns 404 not_uploaded, we open the dialog.
    try {
      const res = await fetch(downloadUrl(os), { method: 'HEAD' })
      if (res.ok) {
        // Real binary exists — trigger the download.
        window.location.href = downloadUrl(os)
        return
      }
      // 404 or other — open the dialog with the fallback.
      setOpen(true)
    } catch {
      setOpen(true)
    }
      ...
      <DialogContent className="sm:max-w-md">
        ...
        <DialogFooter className="flex-col gap-2 sm:flex-col sm:space-x-0">
          <Button variant="outline" className="w-full" asChild>
            <a href="/docs">Build from source</a>
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
