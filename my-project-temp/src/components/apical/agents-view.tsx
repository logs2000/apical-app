...
function ChatPane({ agent, isOrchestrator }: { agent: Workflow | undefined; isOrchestrator: boolean }) {
  ...
  async function sendDo(text: string) {
    ...
    // Real SSE call to /api/agent/think would go here — for now, simulate
    // a short trace so the UI is functional without a live LLM call.
  ...
  return (
    <div className="flex h-full min-h-0 flex-col">
      ...
      <div ref={scrollRef} className="min-h-0 flex-1 space-y-4 overflow-y-auto overscroll-contain p-4">
        {messages.length === 0 && !isOrchestrator && (
          <EmptyState onPick={(p) => send(p)} />
        )}
        {messages.map((m) => (
          <MessageBubble key={m.id} message={m} agentName={isOrchestrator ? "Orchestrator" : agent?.name ?? "Agent"} />
        ))}
        {isThinking && (
      ...
      >
        {/* Mode toggle — Plan vs Do (hidden for Orchestrator) */}
        {!isOrchestrator && (
